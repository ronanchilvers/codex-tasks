...
func parseArguments(args []string, output io.Writer) (config, bool, error) {
	...
	cfg := config{memoryCount: defaultMemoryCount}
	...
	flags.IntVar(&cfg.memoryCount, "memory-count", defaultMemoryCount, "number of recent successful memories to prepend (0 disables injection)")
	...
	if cfg.memoryCount < 0 {
		flags.Usage()
		return config{}, false, errors.New("--memory-count must be zero or greater")
	}
...
func printUsage(output io.Writer, flags *flag.FlagSet) {
	fmt.Fprintln(output, "Usage: codex-task --prompt PATH [--memory-dir PATH] [--memory-count N] [--dry-run] [-- CODEX_OPTIONS...]")
	fmt.Fprintln(output)
